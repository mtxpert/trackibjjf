...
def parse_bracket_html(html, category_id, category_name=""):
    ...
    for match_container in soup.select(".tournament-category__match"):
        ...
        # Override phase if "Final" label detected in match container DOM
        if phase == "ROUND" and re.match(r'Final\b', txt, re.IGNORECASE):
            phase = "FINAL"
        ...
        # Detect placeholder competitors ("Winner/Defeated of Fight X")
        # These live in .match-card__child-where, outside competitor-description
        for ph_el in card.select(".match-card__child-where"):
            ph_text = ph_el.get_text(strip=True)
            ph_m = re.match(r'(Winner|Defeated) of Fight (\d+)', ph_text, re.IGNORECASE)
            if ph_m:
                # Check parent .match-card__competitor for loser class
                parent_comp = ph_el.find_parent(class_=re.compile(r'match-card__competitor\b'))
                loser = bool(parent_comp and
                             "match-competitor--loser" in " ".join(parent_comp.get("class", [])))
                competitors.append({
                    "name": ph_text, "team": "",
                    "loser": loser,
                    "_ph_type": ph_m.group(1).lower(),
                    "_ph_fight": ph_m.group(2),
                })
        ...
        fights.append({
            ...
            "winner":      winner,
            "competitors": [
                {"name": c["name"], "team": c["team"], "loser": c["loser"],
                 **({k: c[k] for k in ("_ph_type", "_ph_fight") if k in c})}
                for c in competitors
            ],
        })

    # ── Resolve placeholder competitors ("Winner/Defeated of Fight X") ────────
    # Process in fight_num order so earlier fights resolve before later ones
    # that depend on them (e.g., "Winner of Fight 5" needs Fight 5 resolved first)
    fights_by_num = {f["fight_num"]: f for f in fights if f["fight_num"]}
    resolve_order = sorted(
        [f for f in fights if f["fight_num"] and f["fight_num"].isdigit()],
        key=lambda x: int(x["fight_num"]),
    ) + [f for f in fights if not f["fight_num"] or not f["fight_num"].isdigit()]
    for f in resolve_order:
        resolved = False
        for c in f["competitors"]:
            ph_type = c.pop("_ph_type", None)
            ph_fight = c.pop("_ph_fight", None)
            if not ph_type:
                continue
            ref = fights_by_num.get(ph_fight)
            if not ref:
                continue
            if ph_type == "winner" and ref["winner"]:
                source = next((rc for rc in ref["competitors"]
                               if rc["name"].lower() == ref["winner"]), None)
            elif ph_type == "defeated":
                source = next((rc for rc in ref["competitors"]
                               if rc.get("loser")), None)
            else:
                source = None
            if source:
                c["name"] = source["name"]
                c["team"] = source["team"]
                resolved = True
        if resolved and len(f["competitors"]) >= 2:
            f["completed"] = any(c.get("loser") for c in f["competitors"])
            if f["completed"]:
                non_losers = [c for c in f["competitors"] if not c["loser"]]
                f["winner"] = non_losers[0]["name"].lower() if len(non_losers) == 1 else ""

    # ── Promote non-SF fights to FINAL ─────────────────────────────────────────
    # Strategy 1: both competitors are SF winners (standard 4+ person brackets)
    if sf_winners and not any(f["phase"] == "FINAL" for f in fights):
        for f in fights:
    ...
    # Strategy 2: highest-numbered fight with 2 real competitors where at least
    # one won an earlier fight (handles repechage brackets where one finalist
    # came through a repechage path rather than a semi-final)
    if not any(f["phase"] == "FINAL" for f in fights):
        all_winners = {f["winner"] for f in fights if f["winner"]}
        candidates = [
            f for f in fights
            if f["fight_num"] and f["fight_num"].isdigit()
            and f["phase"] not in ("SEMI-FINAL", "QUARTER-FINAL")
            and len([c for c in f["competitors"]
                     if c["name"].lower() not in ("bye", "")]) == 2
        ]
        if candidates:
            candidates.sort(key=lambda x: int(x["fight_num"]), reverse=True)
            top = candidates[0]
            comp_names = {c["name"].lower() for c in top["competitors"]}
            if comp_names & all_winners:
                top["phase"] = "FINAL"

    # Strategy 3: small bracket fallback — no SF fights, single real fight
    if not any(f["phase"] == "FINAL" for f in fights):
        non_sf = [
            f for f in fights
        ...
        real = [
            f for f in non_sf
            if len([c for c in f["competitors"]
                    if c["name"].lower() not in ("bye", "")]) == 2
        ]
        if len(real) == 1:
            real[0]["phase"] = "FINAL"

    # ── Cross-validate: correct wrong winners when loser advances to FINAL ────
    # If a competitor in the FINAL was marked as loser in an earlier fight but
    # has no intervening repechage win, the earlier fight's loser flag is wrong.
    final_fights = [f for f in fights if f["phase"] == "FINAL" and f["fight_num"]]
    for final_f in final_fights:
        for fc in final_f["competitors"]:
            name_lower = fc["name"].lower()
            if not name_lower or name_lower in ("bye", ""):
                continue
            for earlier in fights:
                if earlier is final_f or not earlier.get("fight_num"):
                    continue
                if not earlier.get("completed"):
                    continue
                lost_here = any(
                    ec["name"].lower() == name_lower and ec["loser"]
                    for ec in earlier["competitors"]
                )
                if not lost_here:
                    continue
                # Check for repechage path between loss and final
                has_repechage = False
                if earlier["fight_num"].isdigit() and final_f["fight_num"].isdigit():
                    loss_num = int(earlier["fight_num"])
                    final_num = int(final_f["fight_num"])
                    for mid_f in fights:
                        if (mid_f["fight_num"] and mid_f["fight_num"].isdigit()
                                and loss_num < int(mid_f["fight_num"]) < final_num):
                            if any(mc["name"].lower() == name_lower
                                   for mc in mid_f["competitors"]):
                                has_repechage = True
                                break
                if not has_repechage:
                    # No repechage path — loser marking is wrong, flip it
                    for ec in earlier["competitors"]:
                        ec["loser"] = not ec["loser"]
                    non_losers = [ec for ec in earlier["competitors"]
                                  if not ec["loser"]]
                    earlier["winner"] = (non_losers[0]["name"].lower()
                                         if len(non_losers) == 1 else "")

    # Rebuild sf_winners after cross-validation corrections
    sf_winners = set()
    for f in fights:
        if f["phase"] == "SEMI-FINAL" and f["completed"]:
            for c in f["competitors"]:
                if not c["loser"]:
                    sf_winners.add(c["name"].lower())
    ...
    # Use podium data when available — it can fill in missing 1st/2nd place
    # even when fight-derived ranking only captured 3rd-place SF losers.
    medalists = soup.select_one(".tournament-category__medalists, .tournament-category__podium, .podium")
    if medalists:
        official = _parse_medalists(medalists)
        if official:
            # Merge: podium positions take priority, keep fight-derived extras
            official_names = {e["name"] for e in official}
            for r in ranking:
                if r["name"] not in official_names:
                    official.append(r)
            ranking = official
            if any(r["pos"] == "1" for r in ranking):
                results_final = True
...
def _get_phase(match_container):
    ...
    while el and el.name != "body":
        ...
        if "final" in cls.lower() and "semi" not in cls.lower():
            return "FINAL"
        el = el.parent
    return "ROUND"
...
def _parse_medalists(container):
    ...
    results = []
    # Method 1: <li> or class-based entries (including .podium__step)
    entries = container.select("li, .medalist, .podium__step, [class*=medalist], [class*=podium-item]")
    for entry in entries:
        # Prefer structured elements (e.g., .podium__place + .podium__competitor-name)
        place_el = entry.select_one(".podium__place, [class*=place]")
        name_el = entry.select_one(".podium__competitor-name, [class*=competitor-name]")
        if place_el and name_el:
            pos = place_el.get_text(strip=True)
            name = name_el.get_text(strip=True)
            if pos in ("1", "2", "3") and name:
                results.append({"pos": pos, "name": name.lower()})
                continue
    ...
    # Method 2: div-based podium (numbered child containers with name sub-elements)
    children = [c for c in container.children if hasattr(c, 'find_all')]
    for child in children:
        inner = [el.get_text(strip=True) for el in child.find_all(recursive=True)
                 if el.get_text(strip=True)]
        if len(inner) >= 2 and inner[0] in ("1", "2", "3"):
            name = inner[1] if inner[1] not in ("1", "2", "3") else ""
            if name:
                results.append({"pos": inner[0], "name": name.lower()})
    return results
